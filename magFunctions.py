...
#             try:
#                 # if(is_verbose): print('Gathering data from CDAS.')
#                 data = cdas.get_data(
#                     'sp_phys',
#                     'THG_L2_MAG_'+ magname.upper(),
#                     start,
...
#                 #     continue
#             except Exception as e:
#                 print("Trouble with Antarctic mag!!")
#                 print(e)
#                 continue
    ...
    if(is_data_saved): 
        try:
            ...
            for data_list, label in zip([arctic_pwr, antarctic_pwr], ['Arctic', 'Antarctic']):
                ...
                # foo = foo.reset_index()
                # maxiaga = foo[foo['WAVEPWR'] ==max(numeric_data)]['IAGA']
                maxgeolon = foo[foo['WAVEPWR'] ==max(numeric_data)]['GEOLON'][0]
                maxgeolat = foo[foo['WAVEPWR'] ==max(numeric_data)]['GEOLAT'][0]
                maxaacgmlon = foo[foo['WAVEPWR'] ==max(numeric_data)]['AACGMLON'][0]
                maxaacgmlat = foo[foo['WAVEPWR'] ==max(numeric_data)]['AACGMLAT'][0]
